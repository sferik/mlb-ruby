...
module MLB
  class VenuesTest < Minitest::Test
    ...
    def test_self_find
      stub_request(:get, "https://statsapi.mlb.com/api/v1/venues/1?season=#{Time.now.year}&sportId=1")
        .to_return(body: '{"venues":[{"id":1},{"id":2}]}', headers: {"Content-Type" => "application/json;charset=UTF-8"})
      venue = Venues.find(Venue.new(id: 1))

      assert_requested :get, "https://statsapi.mlb.com/api/v1/venues/1?season=#{Time.now.year}&sportId=1"
      assert_equal 1, venue.id
    end
